...
function lifecycle({ mobile = false, webgl = true, fail = false, renderFail = false } = {}) {
  let frames = new Map(), next = 0, draws = 0, disposed = 0, intersection, resized, renderer
  ...
  const ogl = {
    ...
    Program: class { constructor(_gl, options) { this.uniforms = options.uniforms } remove() { disposed++ } },
    Mesh: class {}
  }
  ...
  const ready = []
  const cleanup = context.exports.mountAurora({ clientWidth: 800, clientHeight: 600, appendChild() {}, closest() { return this } }, value => ready.push(value))
  ...
  document.hidden = false; listeners.get('visibilitychange')(); tick(10000)
...
assert.match(css, /\.dark \.hero-aurora \{ opacity: 0\.16; \}/)
assert.match(css, /\.hero-aurora \{\s*opacity: 0\.08;/)
...
const darkWorst = mix([7,7,11], [124,92,255], .16)
// For light theme use an even darker bound than the effect can produce.
const lightWorst = mix([246,248,252], [0,0,0], .08)
...
assert.doesNotMatch(hero, /initial: reduceMotion/)
const wrapper = readFileSync('src/components/effects/HeroAurora.tsx', 'utf8')
assert.match(wrapper, /if \(reducedMotion\) return/)
