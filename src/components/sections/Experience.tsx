...
export function Experience() {
  ...
  return <section id="experience" className="section-space scroll-mt-20 border-y border-border bg-background-secondary/25">
    <div ref={ref} className="mx-auto max-w-editorial px-4 sm:px-6 lg:px-8">
      ...
      <ol className="border-l border-border pl-6 sm:pl-8">{experiences.map((item, index) => <motion.li key={item.slug} initial={reduceMotion ? false : { opacity: 0, x: -12 }} animate={reduceMotion || inView ? { opacity: 1, x: 0 } : {}} transition={reduceMotion ? { duration: 0 } : { delay: index * 0.07, duration: 0.45 }} className="relative pb-9 last:pb-0">
        <span className="absolute -left-[31px] top-1 h-3 w-3 rounded-full border-2 border-background bg-accent sm:-left-[39px]" />
        <p className="text-xs uppercase tracking-[0.15em] text-foreground-tertiary">{item.startDate} - {item.endDate}</p>
        <Link to={`/experiencia/${item.slug}`} className="group mt-2 block focus-visible:rounded-lg">
          <h3 className="text-lg font-semibold group-hover:text-accent">{item.position} <span className="font-normal text-foreground-secondary">· {item.company}</span>
            <ArrowUpRight className="ml-1 inline h-4 w-4 text-accent opacity-0 transition-opacity group-hover:opacity-100" />
          </h3>
          <p className="mt-2 max-w-2xl text-sm text-foreground-secondary">{item.summary}</p>
