...
export function SelectedCases() {
  ...
  return <section id="cases" className="section-space scroll-mt-20">
    <div className="mx-auto max-w-editorial px-4 sm:px-6 lg:px-8">
      <header className="editorial-grid mb-10">
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {selectedCases.map(study => {
          ...
          return <article key={study.id} className="group relative flex min-w-0 flex-col overflow-hidden rounded-2xl border border-border bg-background-secondary shadow-lg shadow-black/10 motion-safe:transition-[transform,box-shadow,border-color] motion-safe:duration-300 motion-safe:hover:-translate-y-1 hover:border-accent/50 hover:shadow-xl hover:shadow-accent/10 focus-within:border-accent">
            <div className="relative overflow-hidden border-b border-border bg-background-tertiary">
              <picture><source media="(min-width: 1024px)" srcSet={desktop.src} /><img src={mobile?.src ?? desktop.src} alt={(mobile ?? desktop).alt[locale]} width={(mobile ?? desktop).width} height={(mobile ?? desktop).height} loading="lazy" decoding="async" className="aspect-[16/10] w-full object-cover object-top motion-safe:transition-transform motion-safe:duration-500 motion-safe:group-hover:scale-[1.03]" /></picture>
            </div>
