...
export function ExperienceProjectBlock({ project, index, labels }: ExperienceProjectBlockProps) {
  ...
  const visual = <div className="lg:col-span-6">{project.media.length === 0 ? <ExperiencePlaceholder label={labels.pendingCapture(project.name)} /> : project.media.length === 1 ? <DeviceShowcase media={project.media[0]} onOpen={() => setActiveIndex(0)} openLabel={labels.openImage} /> : <ExperienceCarousel media={project.media} onOpen={setActiveIndex} labels={{ carousel: labels.carousel, previous: labels.previousView, next: labels.nextView, goTo: labels.goToView, counter: labels.viewCounter, openImage: labels.openImage }} />}</div>
  ...
  return <><motion.article {...motionProps} className="grid grid-cols-1 items-center gap-8 border-t border-border py-12 first:border-t-0 first:pt-0 lg:grid-cols-12 lg:gap-12">{imageFirst ? <>{visual}{copy}</> : <>{copy}{visual}</>}</motion.article><ImageLightbox images={images} activeIndex={activeIndex} onClose={() => setActiveIndex(null)} onNavigate={setActiveIndex} closeLabel={labels.lightboxClose} previousLabel={labels.lightboxPrevious} nextLabel={labels.lightboxNext} /></>
}
