...
export function ExperienceProjectBlock({ project, index }: ExperienceProjectBlockProps) {
  ...
  const images = useMemo(() => project.media.flatMap((media) => media.mobile ? [media.desktop, media.mobile] : [media.desktop]), [project.media])
  const onOpen = useCallback((image: Screenshot) => setActiveIndex(images.findIndex((candidate) => candidate.src === image.src)), [images])
  ...
  const visual = <div className="lg:col-span-6">{project.media.length ? <div className="space-y-6">{project.media.map((media) => <DeviceShowcase key={media.desktop.src} media={media} mobileOnLeft={imageFirst} onOpen={onOpen} />)}</div> : <ExperiencePlaceholder label={`Captura pendiente de ${project.name}`} />}</div>
