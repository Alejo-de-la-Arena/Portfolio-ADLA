...
export const experience: Experience[] = [
  ...
  },
  {
    ...
    location: 'Argentina · Remoto',
    summary:
      'Diseño y desarrollo end-to-end de la plataforma web de una empresa de cocinas a medida y reformas integrales en Zona Norte del GBA y CABA. El proyecto combina un portfolio editorial de obras reales con un panel de administración propio, para que el equipo publique y gestione su contenido sin depender de desarrollo. La conversión se resuelve enteramente por WhatsApp: cada formulario arma el mensaje con el contexto del lead ya redactado. Incluye además toda la capa de infraestructura: deploy, dominios, DNS y correo corporativo. Proyecto en producción.',
    websiteUrl: 'https://www.renovatucocina.com.ar',
    images: [],
    achievements: [
      'Panel de administración con CRUD completo de proyectos, subida de imágenes y control de visibilidad, destacados y orden desde la propia tabla.',
      'Conversión sin backend de formularios: cada envío arma un mensaje estructurado según el motivo de consulta y abre WhatsApp con los datos del lead ya cargados.',
      'Capa de datos con fallback: si Supabase no responde, las queries caen a un dataset estático y el sitio sigue sirviendo contenido.',
      'Infraestructura completa: dos dominios en registradores distintos, nameservers delegados, zona DNS administrada y redirecciones 308 al dominio canónico.',
    ],
    technologies: [
      'Next.js 15',
      'React',
    ...
    details: {
      built: [
        'Sitio público con 13 rutas entre páginas, panel y route handlers. Home editorial con selector de tipos de renovación, sección de cocina desde cero y timeline de proceso.',
        'Página de Reformas para obra estructural con dirección de arquitecto: acordeón interactivo de servicios, proceso de 5 etapas, showcase de transformaciones y FAQ propio.',
        'Listado de proyectos con filtros combinables resueltos en servidor, y detalle con hero en crossfade, comparador antes/después, bloque de reto/solución y galería editorial.',
        'Formulario de contacto multi-paso con tres variantes (renovar, desde cero, consulta general), cada una con sus propios campos y validación.',
        'Panel privado con CRUD de proyectos, formulario dividido en bloques (datos técnicos, imágenes, SEO, galería) y subida a Supabase Storage vía API route protegida con rutas namespaceadas por usuario.',
        'Auth con sesión por cookies y middleware sobre /admin/* que preserva la ruta destino después del login.',
      ],
      optimized: [
        'Mutaciones del panel implementadas como server actions con revalidación selectiva de las rutas afectadas: el cliente publica una obra y aparece en el sitio sin redeploy.',
        'ISR de 60 segundos en las rutas públicas para balancear contenido fresco con performance estática.',
        'Metadata SEO por proyecto editable desde el panel, con valores de respaldo cuando el campo queda vacío.',
        'Optimización de imágenes con next/image, sitemap y robots dinámicos generados a partir de los slugs en base.',
      ],
      decisions: [
        'Resolver la conversión por WhatsApp en lugar de montar un backend de formularios: el canal donde el equipo ya atendía, con la consulta llegando completa en el primer mensaje y sin infraestructura de email que mantener.',
        'Capa de mapeo bidireccional entre las columnas snake_case de Postgres y el modelo que consumen los componentes, para que un cambio de esquema no se filtre a la UI.',
        'Fallback a dataset estático en la capa de queries: el sitio nunca se cae por un problema de base de datos.',
        'Definición del dominio canónico y redirecciones 308 entre las variantes para no partir autoridad SEO entre los dos dominios.',
      ],
      results: [
        'Cliente con autonomía total sobre su contenido: publica, edita y ordena sus obras desde el panel sin intervención de desarrollo.',
        'Sitio en producción con CI/CD desde GitHub, correo corporativo propio sobre Google Workspace y medición activa vía Google Tag Manager y Meta Pixel.',
      ],
      links: [
        { label: 'Sitio', href: 'https://www.renovatucocina.com.ar' },
      ],
    },
  },
  {
